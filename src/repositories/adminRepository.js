...
const productsStatusEnumHasPending = async () => {
  const [rows] = await pool.query(
  ...
  return String(rows[0].ct).includes("'PENDING'");
};
...
const initAdminTables = async () => {
  ...
  if (!(await productsStatusEnumHasPending())) {
    await pool.query(
  ...
  if (await columnExists("products", "seller_id")) {
    if (!(await columnExists("products", "buy_now_price"))) {
      await pool.query(
        "ALTER TABLE products ADD COLUMN buy_now_price DECIMAL(15, 2) DEFAULT NULL",
      );
    }
    if (!(await columnExists("products", "bid_increment"))) {
      await pool.query(
        "ALTER TABLE products ADD COLUMN bid_increment DECIMAL(15, 2) DEFAULT NULL",
      );
    }
    if (!(await columnExists("products", "deposit_required"))) {
      await pool.query(
    ...
    if (!(await columnExists("products", "video_url"))) {
      await pool.query(
...
const createProductModeration = async (
  productId,
  status = "PENDING",
  adminId = null,
  reason = null,
...
const getModerationProducts = async ({ status, query, offset, limit }) => {
  let sql = `SELECT p.id,
                      ...
                      u.email AS seller_email,
                      c.name AS category_name,
                      COALESCE(pm.status, CASE WHEN p.status IN ('DRAFT', 'PENDING') THEN 'PENDING' ELSE 'APPROVED' END) AS moderation_status,
